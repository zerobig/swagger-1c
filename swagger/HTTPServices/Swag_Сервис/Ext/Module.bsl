...
Функция SwaggerFileGET(Запрос)
	...
	Попытка
		...
		Если НаименованиеМакета = "Swag_swagger_json" Тогда
			...
			Ответ.УстановитьТелоИзСтроки(СформироватьБазовыйSwaggerJson(СтрЗаменить(Запрос.БазовыйURL, "/swagger", "")),
		...
		Иначе
			...
			Если Прав(НаименованиеМакета, 5) = "_html" Тогда
				...
				Ответ.УстановитьТелоИзСтроки(СтрЗаменить(Данные, "{БазовыйURL}", Запрос.БазовыйURL),
					КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
	...
	Для каждого HTTPСервис Из Метаданные.HTTPСервисы Цикл
		...
		Для каждого ШаблонURL Из HTTPСервис.ШаблоныURL Цикл
			...
			Для каждого Метод Из ШаблонURL.Методы Цикл
				...
				ОписаниеМетода = ПолучитьОписаниеМетода(Описание, ШаблонURL.Имя + Метод.HTTPМетод);
				...
				Для каждого ПараметрМетода Из ПараметрыМетода Цикл
					...
					ЗаписьJSON.ЗаписатьИмяСвойства("allowEmptyValue");
					ЗаписьJSON.ЗаписатьЗначение(ПараметрМетода.ПринимаютсяПустыеЗначения);
					ЗаписьJSON.ЗаписатьКонецОбъекта();
...
Функция ПолучитьОписаниеМетода(Описание, Метод)
	...
	Если ТипЗнч(Описание) = Тип("Массив") Тогда
		...
		Для каждого ДанныеОписания Из Описание Цикл
			Если ТипЗнч(ДанныеОписания) = Тип("Структура") И ДанныеОписания.Свойство("Имя") Тогда
				Если ДанныеОписания.Имя = Метод И ДанныеОписания.Свойство("Описание") Тогда
					ОписаниеМетода = ДанныеОписания.Описание;
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
