	...
	Попытка
		...
		Иначе
			...
			Если Прав(НаименованиеМакета, 5) = "_html" Тогда
				...
				Если Запрос.ПараметрыЗапроса.Получить("ui") = "redoc" Тогда
					// https://github.com/Redocly/redoc/issues/1222
					Макет = ПолучитьОбщийМакет("Swag_redoc_index_html");
				ИначеЕсли Запрос.ПараметрыЗапроса.Получить("ui") = "scalar" Тогда
					Макет = ПолучитьОбщийМакет("Swag_scalar_index_html");
				ИначеЕсли Запрос.ПараметрыЗапроса.Получить("ui") = "stoplight" Тогда
					Макет = ПолучитьОбщийМакет("Swag_stoplight_index_html");
				ИначеЕсли Запрос.ПараметрыЗапроса.Получить("ui") = "rapidoc" Тогда
