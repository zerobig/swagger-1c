...
Функция ПолучитьJsonRpcFileGET(Запрос) Экспорт
	...
	Попытка
		...
		Если НаименованиеМакета = "Swag_json_rpc_json" Тогда
				
			Ответ.Заголовки.Вставить("Content-Type", "application/json");
			Ответ.УстановитьТелоИзСтроки(СформироватьJsonRpc(СтрЗаменить(ПолучитьБазовыйURLСПортом(Запрос), "/swagger", "")),
				КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);

		Иначе
			Если Прав(НаименованиеМакета, 5) = "_html" Тогда

				Макет = ПолучитьОбщийМакет("Swag_playground_rpc_index_html");
...
Функция СформироватьJsonRpc(БазовыйURL)
