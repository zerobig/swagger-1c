...
Функция Свойство(ИмяСвойства) Экспорт
	...
	МассивСвойств.Добавить(Новый Структура("ИмяСвойства, Описание, Тип, Nullable, Формат, Схема, Пример",
		ИмяСвойства, , , Ложь));
...
Функция ТипЗначения(Тип) Экспорт
	...
	КонецЕсли;
	Если Не НРег(Тип) = "integer" И Не НРег(Тип) = "string" И
		Не НРег(Тип) = "boolean" И Не НРег(Тип) = "object" И Не НРег(Тип) = "array" Тогда
		
		ВызватьИсключение "ТипЗначения: тип свойства должен быть одним из следующего списка - integer, string, boolean, object или array";
	КонецЕсли;
...
Процедура ДобавитьСвойстваВОбъект(ТекущийОбъект)
	...
	Если СтруктураОписания.Свойство("МассивСвойств") И
		...
		Для каждого СвойствоОбъекта Из СтруктураОписания.МассивСвойств Цикл
			...
			//Если СвойствоОбъекта.Тип = Неопределено Тогда
			//	ВызватьИсключение СтрШаблон("Сформировать: для свойства %1 не задан тип",
