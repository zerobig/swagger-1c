...
Функция Метод(ИмяМетода) Экспорт
	...
	СтруктураОписания.Вставить("ТекущийМетод",
		Новый Структура("Имя, Описание, Параметры, Ответы, ТелоЗапроса, ДетальноеОписание", ИмяМетода));
...
//
//
Функция ДетальноеОписаниеМетода(СтрокаОписания) Экспорт
	...
	СтруктураОписания.ТекущийМетод.ДетальноеОписание = СтрокаОписания;
...
Функция МаксимальнаяДлина(Значение) Экспорт
	УстановитьЗначениеСвойстваПараметра("МаксимальнаяДлина", Значение, "МаксимальнаяДлина");
	Возврат ЭтотОбъект;
КонецФункции
...
Функция МинимальнаяДлина(Значение) Экспорт
	УстановитьЗначениеСвойстваПараметра("МинимальнаяДлина", Значение, "МинимальнаяДлина");
	Возврат ЭтотОбъект;
КонецФункции
		...
		Если Ответ.Код = СтруктураОписания.КодТекущегоОтвета Тогда
			Для каждого ТипMIME Из Ответ.МассивТиповMIME Цикл
				Если ТипMIME.Заголовок = СтруктураОписания.ЗаголовокТекущегоОтвета Тогда
					Если ТипЗнч(ТипMIME.Схема) = Тип("Структура") Тогда
						МассивТиповСхемы = ТипMIME.Схема.МассивТиповСхемы;
						МассивТиповСхемы.Добавить(СтрШаблон("#/components/schemas/%1", ТипОбъекта));
						
						ТипMIME.Схема.МассивТиповСхемы = МассивТиповСхемы;
					Иначе
...
//
//
Функция ОбъединениеСхемОтвета(СпособОбъединения) Экспорт

	ПроверитьДанныеОтвета("ОбъединениеСхемОтвета");
	Если Не СтруктураОписания.Свойство("ЗаголовокТекущегоОтвета") Или
		ПустаяСтрока(СтруктураОписания.ЗаголовокТекущегоОтвета) Тогда
		
		ВызватьИсключение "ОбъединениеСхемОтвета: нельзя задавать объединение схем ответа для неопределенного типа ответа";
	КонецЕсли;
	...
	Для каждого Ответ Из СтруктураОписания.ТекущийМетод.Ответы Цикл
		Если Ответ.Код = СтруктураОписания.КодТекущегоОтвета Тогда
			Для каждого ТипMIME Из Ответ.МассивТиповMIME Цикл
				Если ТипMIME.Заголовок = СтруктураОписания.ЗаголовокТекущегоОтвета Тогда
					ТипMIME.Схема = Новый Структура("СпособОбъединенияСхем, МассивТиповСхемы",
...
//
//
Функция ОбъединениеСхемТела(СпособОбъединения) Экспорт
...
Функция Сформировать() Экспорт
	...
	Если СтруктураОписания.Свойство("МассивМетодов") И
		...
		Для каждого Метод Из СтруктураОписания.МассивМетодов Цикл
			...
			Описание = Метод.Описание;
			Если ЗначениеЗаполнено(Метод.ДетальноеОписание) Тогда
				Описание = Новый Структура("Описание, ДетальноеОписание",
					Метод.Описание, Метод.ДетальноеОписание);
			КонецЕсли;
			
			МассивОписанийМетодов.Добавить(
				Swag_ФормированиеОписаний.ОписаниеМетода(Метод.Имя, Описание,
...
Процедура ДобавитьПараметрВМассивПараметров(ИмяПараметра, ТипПараметра, НазваниеПроцедуры)
	...
	МассивПараметров.Добавить(Новый Структура("Имя, Описание, ТипПараметра, Обязательный,
			|Устарел, ТипЗначения, Перечисление, МаксимальнаяДлина, МинимальнаяДлина",
